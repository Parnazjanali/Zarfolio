...
    const path = location.pathname;
    // +++ بروزرسانی منطق برای فعال ماندن آیتم‌های زیرمنوی افزونه‌ها +++
    if (path.startsWith('/plugins') || path.startsWith('/purchase-plugin')) {
        return [path]; // این باعث می‌شود آیتم دقیق داخل زیرمنو انتخاب شود
    }
...
    }
    // +++ اضافه کردن منطق برای باز نگه داشتن زیرمنوی افزونه‌ها +++
...
    { key: '/customers', icon: <FaUsers />, label: <Link to="/customers">مشتریان</Link> },
    // +++ جایگزینی دکمه تکی با زیرمنوی کامل افزونه‌ها +++
    {
      key: 'pluginsSubmenu',
      icon: <FaPlug />,
      label: 'افزونه‌ها',
