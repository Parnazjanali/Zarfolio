...
func (s *userService) RegisterUser(req model.RegisterRequest) error {
	...
	newUser := &model.User{
		...
		Email:        req.Email,
		// ✅ **تغییر اصلی:** به جای "user"، از نقش پیش‌فرض "فروشنده" که در مدل تعریف کردیم استفاده می‌شود.
		Role:         model.Seller,
	}
...
func (s *userService) AuthenticateUser(username, password string) (*model.User, string, *utils.CustomClaims, error) {
	...
	utils.Log.Info("User authenticated successfully in service", zap.String("username", user.Username), zap.String("role", string(user.Role)))
