...
func NewAuthZMiddleware(ps *authz.PermissionService, logger *zap.Logger, jwtValidator utils.JWTValidator) *AuthZMiddleware { // <--- نام ورودی را به jwtValidator تغییر دهید
    if ps == nil {
        logger.Fatal("PermissionService cannot be nil for AuthZMiddleware in Profile Manager.")
    }
    if logger == nil {
        panic("Logger cannot be nil for AuthZMiddleware in Profile Manager.")
    }
    if jwtValidator == nil { 
        panic("JWTValidator cannot be nil for AuthZMiddleware in Profile Manager.")
    }
...
func (m *AuthZMiddleware) VerifyInternalToken() fiber.Handler {
    return func(c *fiber.Ctx) error {
        ...
        if requestedUserID != "" && requestedUserID != claims.UserID {
            ...
            if !m.permissionService.HasPermission(userRoles, model.PermUserChangeAnyPassword) &&
                !m.permissionService.HasPermission(userRoles, model.PermUserUpdate) {
