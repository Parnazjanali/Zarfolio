...
func (h *AuthHandler) HandleLoginTwoFA(c *fiber.Ctx) error {
	...
	if err != nil {
		// Errors like UserNotFound or service internal errors during VerifyTOTP
		utils.Log.Error("HandleLoginTwoFA: Error calling userService.VerifyTOTP", zap.String("userID", req.UserID), zap.Error(err))
	...
	if !isValid {
		utils.Log.Warn("HandleLoginTwoFA: Invalid TOTP code provided", zap.String("userID", req.UserID))
		return c.Status(fiber.StatusUnauthorized).JSON(model.ErrorResponse{
			Message: "Invalid 2FA code.", Code: "401_INVALID_2FA_CODE",
		})
	}

	finalUser, err := h.userService.GetUserByIDForTokenGeneration(req.UserID) // NEW service method needed
	if err != nil {
