...
func SetuUpAuthRoutes(app *fiber.App, authHandler *handler.AuthHandler, authMiddleware *middleware.AuthZMiddleware) error {
	if app == nil {
	...
	authGroup.Post("/logout", authMiddleware.AuthorizeMiddleware(model.PermUserRead), authHandler.Logout)
