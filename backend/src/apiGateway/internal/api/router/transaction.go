...
func SetUpTransactionRoutes(app *fiber.App, transactionHandlerAG *handler.TransactionHandler, authMiddleware *middleware.AuthMiddleware, logger *zap.Logger) error {
	defer logger.Sync()

	if app == nil {
		logger.Error("Fiber app instance cannot be nil in TransactionManager.")
		return fmt.Errorf("fiber app instance cannot be nil in TransactionManager.")
	}
	if transactionHandlerAG == nil {
		logger.Error("TransactionHandlerAG is nil in TransactionManager's SetUpTransactionRoutes.")
	...
	integrationGroup.Post("/sync", 
		...
		transactionHandlerAG.HandleManualSync)

	// ----------------------
	// 16. روت‌های مدیریتی (Admin)
	// ----------------------
	// */
	
	adminGroup := transactionGroup.Group("/admin")
	adminGroup.Use(authMiddleware.AuthorizeMiddleware(model.PermAdminFullAccess))
	
	adminGroup.Get("/audit", transactionHandlerAG.HandleGetAuditLog)
	//adminGroup.Post("/cleanup", transactionHandlerAG.HandleCleanupOldTransactions)
	//adminGroup.Post("/recalculate", transactionHandlerAG.HandleRecalculateBalances)

	logger.Info("Transaction routes set up successfully with auth middleware.")
	return nil
}